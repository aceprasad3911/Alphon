...
def setup_directories():
    ...
    dirs = [ #TODO: Ammend once all data streams are fully operationalized
...
def db_run_command(cmd, check=True):
...
def db_setup():
    ...
    try:
        db_run_command(["brew", "services", "start", "postgresql"], check=False)
    ...
    db_run_command(["python", str(DB_CLIENT)]) #TODO: make sure db client also links DB & is connected to main script


    # 6. Seed data
    print("🌱 Inserting seed data...")
    db_run_command(["python", str(SEED_DATA)])
    ...
    print("🎯 Database initialization complete!") #TODO: Check Functionality of DB Init Processes and add DB linkage/Display
